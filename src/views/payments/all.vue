...
const fetchData = async () => {
  await getAllPayments(status.value, 1);
...
const page_title = ref({ title: "Payments" });
const note = ref("");
  ...
  <div>
    ...
    <div class="mt-4">
        ...
        <v-col cols="12" sm="6" md="4">
          <v-card elevation="0" class="pa-6 h-100">
            ...
            <div v-if="withdrawals_total?.length > 0" class="mt-11">
              <h2 class="mb-2">
                ₦‎{{ payments?.meta?.sum ? formatCurrency(payments?.meta?.sum) : "0" }}
              </h2>
              <span>All time</span>
            </div>
          </v-card>
            ...
            <div class="ml-16 d-flex align-center justify-space-between w-100">
              <div class="d-flex align-start justify-start flex-column w-100">
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{
                      formatCurrency(payments?.meta?.success)
                    }}
                  </h2>
              ...
              >
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{ formatCurrency(payments?.meta?.pending) }}
                  </h2>
                  <span>pending</span>
                </div>
              </div>
              ...
              >
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{ formatCurrency(payments?.meta?.failed) }}
                  </h2>
                  <span>Failed</span>
                </div>
              </div>
      ...
      <v-card class="pa-5">
                    ...
                    <v-menu transition="scroll-y-transition">
                      ...
                      <v-list>
                        ...
                        <v-list-item
                          ...
                          @click="approveRequest(payment?.id)"
                          link
                          color="secondary"
                        >
                          <v-list-item-title>
        ...
        <p
          v-if="loading == false && payments?.data?.length <= 0"
      ...
      <v-pagination
        ...
        @next="getAllPayments(status, page)"
        @prev="getAllPayments(status, page)"
        @update:modelValue="getAllPayments(status, page)"
        active-color="red"
        :start="1"
        variant="flat"
        class="mt-5"
