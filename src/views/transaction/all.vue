...
const fetchData = async () => {
  await getAllTransactions(status.value, 1);
...
const page_title = ref({ title: "Transactions" });
const note = ref("");
  ...
  <div>
    ...
    <div class="mt-4">
        ...
        <v-col cols="12" sm="6" md="4">
          <v-card elevation="0" class="pa-6 h-100">
            ...
            <div v-if="withdrawals_total?.length > 0" class="mt-11">
              <h2 class="mb-2">
                ₦‎{{ formatCurrency(all_transaction?.meta?.sum) }}
              </h2>
              <span>All time</span>
            </div>
          </v-card>
            ...
            <div class="ml-16 d-flex align-center justify-space-between w-100">
              <div class="d-flex align-start justify-start flex-column w-100">
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{
                      formatCurrency(all_transaction?.meta?.success)
                    }}
                  </h2>
              ...
              >
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{ formatCurrency(all_transaction?.meta?.pending) }}
                  </h2>
                  <span>pending</span>
                </div>
              </div>
              ...
              >
                ...
                <div v-if="withdrawals_total?.length > 0" class="pl-3 my-5">
                  <h2 class="mb-2">
                    {{ formatCurrency(all_transaction?.meta?.failed) }}
                  </h2>
                  <span>Failed</span>
                </div>
              </div>
      ...
      <v-card class="pa-5">
        ...
        <v-window v-model="tab">
          <v-table>
            ...
            <tbody v-if="loading == false">
              <tr
                class="pa-3"
                v-for="(transaction, index) in all_transaction?.data"
                :key="transaction?.id"
              >
                <td>{{ index + 1 }}</td>
        ...
        <p
          v-if="loading == false && all_transaction?.data?.length <= 0"
      ...
      <v-pagination
        ...
        @next="getAllTransactions(status, page)"
        @prev="getAllTransactions(status, page)"
        @update:modelValue="getAllTransactions(status, page)"
        active-color="red"
        :start="1"
        variant="flat"
        class="mt-5"
